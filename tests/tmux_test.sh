...
#{session_gropu_size}:#{session_group_list}:#{session_grouped}:#{session_id}:\
...
#{pane_height}'#{pane_id}'#{pane_in_mode}'#{pane_input_off}'#{pane_index}'\
#{pane_left}'#{pane_mode}'#{pane_pid}'#{pane_pipe}'#{pane_right}'\
#{pane_search_string}'#{pane_start_command}'#{pane_synchronized}'#{pane_tabs}'\
#{pane_title}'#{pane_top}'#{pane_tty}'#{pane_width}"
